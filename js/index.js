    ...
    fetch('./products.json').then(function(response) {
        response.json().then(function(products) {
            ...
            if (products[productType]) {
                ...
                for (product in products['adjusting_instrument']['products']) {
                    let productHTML = `
                        <div class="col-md-3 col-6 px-5 mb-4">
                            <img class="rounded w-100 mb-3" src="./assets/${products['adjusting_instrument']['products'][product]['image']}">
                            <h5 class="text-center text-dark">${products['adjusting_instrument']['products'][product]['name']}</h5>
                        </div>
                    `;
                    productsContainer.innerHTML += productHTML;
                }
